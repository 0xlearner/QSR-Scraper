...
class ChargrillCharliesTransformer(TransformerInterface):
    """
    Transformer for Chargrill Charlie's data.
    Responsible for standardizing location data and extracting shopping centre information.
    """
    ...
    def _get_shopping_centre_patterns(self) -> List[str]:
        ...
        return [
            ...
            r"([\w\s]+Food Court)",
            r"([\w\s]+Waterside)",
            r"(Forestway Shopping Centre)",  # Specific to Chargrill Charlie's data
            r"(South Gate Shopping Centre)",  # Specific to Chargrill Charlie's data
        ]
    ...
    async def transform(
        ...
        """
        Transform Chargrill Charlie's location data into standardized format.
        ...
        """
        logger.info(f"ChargrillCharliesTransformer starting transformation of {len(data)} items")
        ...
        for item in data:
            try:
                ...
                brand = item.get('brand', 'Chargrill Charlie\'s')
                ...
                source = item.get('source', 'chargrillcharlies')
                
                # Extract shopping centre information
                shopping_centre = self._extract_shopping_centre(raw_address)
                
                # Clean the address
                street_address = self._clean_address(raw_address, shopping_centre)
                
                # Normalize state
                normalized_state = self._normalize_state(state)
                
                # Generate business ID
                business_id = self._generate_business_id(business_name, street_address)
                ...
                logger.debug(f"Transformed location: {business_name} -> {transformed_dict}")
                
            except Exception as e:
                logger.error(f"Error transforming location {item}: {e}")
                continue
        
        logger.info(f"ChargrillCharliesTransformer completed. Transformed {len(transformed_locations)} locations")
