...
class OportoTransformer(TransformerInterface):
    """
    Transformer for Oporto data.
    ...
    async def transform(
        ...
        """
        Transforms raw Oporto data from parser.

        Args:
            data: List of raw store items from Oporto parser
            config: Transformation configuration
            site_name: Source name (should be 'oporto')

        Returns:
            List of transformed location dictionaries
        """
        if not data:
            logger.warning("No data to transform for Oporto")
            return []
        ...
        for item in data:
            try:
                ...
                logger.error(f"Error transforming Oporto item: {e}", exc_info=True)
                continue

        logger.info(f"Transformed {len(transformed_items)} Oporto items")
    ...
    def _extract_shopping_centre_name(self, street_address: str) -> Optional[str]:
        ...
        shopping_centre_patterns = [
            ...
            r'(.+?)\s*Arcade',
            r'(.+?)\s*Emporium',
            r'(.+?)\s*Home\s*Central',  # For Oporto specific patterns
            r'(.+?)\s*HomeCentre',
        ]
    ...
    def _clean_shopping_centre_name(self, name: str) -> str:
        ...
        prefixes_to_remove = [
            ...
            r'^Showroom\s+[A-Za-z0-9]+\s*,?\s*',
            r'^\d+[A-Za-z]?\s*,?\s*',  # Just numbers
            r'^Food\s+Court\s+Shop\s+\d+\s*',  # Food Court Shop prefix
        ]
