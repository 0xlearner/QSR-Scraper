        ...
        try:
            async with aiofiles.open(output_file, mode='a', encoding='utf-8') as f:
                for item in data:
                    ...
                    json_string = json.dumps(item, ensure_ascii=False, default=json_serializer)
                    await f.write(json_string + '\n')
            logger.info(f"Successfully appended {len(data)} items to {output_file}")
        except TypeError as e:
             # Catch potential new serialization errors
             logger.error(f"JSON serialization error saving to {output_file}: {e} - Check item structure.", exc_info=True)
        except IOError as e:
            logger.error(f"Error writing to JSON file {output_file}: {e}")
        except Exception as e:
            logger.error(f"Unexpected error saving to JSON file {output_file}: {e}", exc_info=True) # Add exc_info for detail
